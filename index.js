...
bot.on('message', async (msg) => {
    ...
    const secret_password = config_data.password;

    let is_unlocked = checkSession();

    /* ================= 🔥 UNBLOCK FIX (IMPORTANT) ================= */

    if (text_lower === "unblock bot") {

        try { fs.unlinkSync(blocked_file); } catch (e) {}
        try { fs.unlinkSync(attempts_file); } catch (e) {}
    ...
    if (text === secret_password) {
        ...
        await bot.sendMessage(chatId, "🔓 Bot Unlocked");
        return;
    }
...
const token = "8739567989:YOUR_BOT_TOKEN_HERE";
const my_chat_id = "5429869370";
