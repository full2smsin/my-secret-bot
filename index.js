...
// क्रैश और डुप्लिकेट पोलिंग ब्लॉक से बचाने के लिए ग्लोबल एरर हैंडलर
process.on('uncaughtException', (err) => console.error('Uncaught Exception:', err.message));
process.on('unhandledRejection', (reason, p) => console.error('Unhandled Rejection:', reason));

const token = '7541620242:AAE65mYvP9jGAs4u8EksB_mS4Xp-L5O2oVs';

// Polling को सुरक्षित रखने के लिए सेटिंग्स (ताकि अलर्ट कभी मिस न हों)
...
// पोलिंग एरर लॉग करने के लिए (ताकि रेंडर कंसोल में एरर दिखे)
bot.on('polling_error', (error) => {
    console.error('Telegram Polling Error Details:', error.message);
});
...
app.get('/download-vault-file', async (req, res) => {
    ...
    try {
        ...
        console.error("Green API endpoint error:", error.message);
        res.status(500).send("Error uploading to Green API");
    }
});

app.get('/', (req, res) => res.send('Cron-backed Vault Server is active!'));
