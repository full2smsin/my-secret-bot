...
// 🔒 ✅ फिक्स्ड डिक्रिप्शन फंक्शन (वेरिएबल ओवरराइट बग को पूरी तरह ठीक कर दिया गया है)
function decryptData(encryptedText, keyPassword) {
    try {
        ...
        const actualEncryptedStr = parts[2]; // फिक्स: केवल तीसरे इंडेक्स का स्ट्रिंग डेटा लिया
        
        const key = crypto.scryptSync(keyPassword, salt, 32);
        const decipher = crypto.createDecipheriv('aes-256-cbc', key, iv);
        
        let decrypted = decipher.update(actualEncryptedStr, 'hex', 'utf8');
...
// 🟢 ✅ फिक्स्ड: मेटा व्हाट्सएप एपीआई को मीडिया भेजने का फंक्शन (URL और सिंटैक्स एरर फ्री)
async function sendToWhatsAppMeta(targetMobile, fileId, type, fileName) {
    try {
        ...
        if (type === "photo") {
            ...
            mediaObject.filename = `${fileName}.${ext}`; // डॉक्यूमेंट पेलोड में कोई 'caption' नहीं डाला गया
        }
        ...
        // यदि मेटा एरर दे तो डिबगिंग के लिए रेंडर टर्मिनल पर प्रिंट करें
        if (!response.ok) {
            console.log("META REJECTION RESPONSE:", await response.json());
        }
            ...
            for (let key of target_keys) {
                if (vault[key]) {
                    ...
                    if (decrypted_file_id) {
                        ...
                        // व्हाट्सएप ट्रांसफर मोड एक्टिवेट करना (यहाँ फ़ाइल आईडी सेव कर रहे हैं)
